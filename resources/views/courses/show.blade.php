                ...
                <div class="card-body">
                    ...
                    <ul class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                        @foreach($course->goals as $goal)
                            <li class="text-gray-700 text-base"><i class="far fa-check-circle text-blue-600 mr-2"></i>{{ $goal->name }}</li>
                        @endforeach
                    </ul>
                </div>
            </section>
                ...
                <div class="card-body">
                    ...
                    @else
                        <form action="{{ route('courses.enrolled', $course) }}" method="post">
                            @csrf
                            <button class="btn btn-danger btn-block mt-4" type="submit">Apuntarme a este curso</button>
                        </form>
                    @endcan
                </div>
            </section>
            <aside class="hidden lg:block">
                @foreach($similars as $similar)
                    <article class="flex mb-6">
                        <img class="h-32 w-45 object-cover" src="{{ Storage::url($similar->image->url) }}" alt="img">
                        <div class="ml-3">
                            <h1>
